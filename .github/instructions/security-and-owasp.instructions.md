...
## AWS Knowledge Consultation for Security
- **MANDATORY**: Before implementing any security measures involving AWS services, consult the AWS Knowledge base MCP server
- Query for AWS security best practices: "AWS IAM least privilege", "AWS Lambda security", "AWS S3 bucket security policies", "AWS Parameter Store encryption"
- Research AWS security services: "AWS WAF configuration", "AWS CloudTrail logging", "AWS GuardDuty threat detection"
- Validate security implementations against AWS Well-Architected Security Pillar guidelines
- Include AWS security documentation references in code comments for critical security implementations
...
- **Prevent Cross-Site Scripting (XSS):** 
  - **Frontend:** When generating frontend code that displays user-controlled data, you must use context-aware output encoding. Prefer methods that treat data as text by default (`.textContent`) over those that parse HTML (`.innerHTML`). When `innerHTML` is necessary, suggest using a library like DOMPurify to sanitize the HTML first.
  - **Backend/Email Templates:** When inserting user data into HTML templates (emails, server-rendered pages), always HTML-escape the data:
    ```javascript
    ...
    // Use before inserting into HTML
    const safeContent = template.replace(/{{USER_INPUT}}/g, escapeHtml(userInput));
    ```
